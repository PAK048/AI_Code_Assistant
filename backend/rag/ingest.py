...
def embed_texts(texts: List[str]) -> List[List[float]]:
    ...
    try:
        ...
        print(f"⚠️  Watsonx embedding failed: {e}")
        print(f"   Using fallback hash-based embeddings (384 dimensions)")
        # Simple deterministic fallback based on text hash
        return [[(hash(t + str(i)) % 1000) / 1000 for i in range(384)] for t in texts]
